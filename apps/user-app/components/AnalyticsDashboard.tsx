    ...
    return (
      <div
        className="rounded-lg border border-slate-200
                   bg-white/90 p-2 text-sm
                   shadow-md backdrop-blur-sm"
      >
        <p className="text-slate-900 font-medium">{`Value: ${payload[0].value}`}</p>
      </div>
    );
  }
  return null;
...
function StatCard({ title, value, change, changeType, icon: Icon, chartData }: any) {
  const chartColor = changeType === 'positive' ? '#22c55e' : '#ef4444';
...
export default function AnalyticsDashboard() {
  return (
    <div className="w-full">
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-6 border-b border-slate-200 gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">
            Analytics Overview
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            Real-time performance metrics for your account.
          </p>
        </div>
      </header>
